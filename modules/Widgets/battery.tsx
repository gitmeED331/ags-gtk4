...
const ChargeIndicatorIcon = ({ battery, charging }: { battery: AstalBat.Device; charging: any }) => (
	<image cssClasses={[bind(battery, "charging").as((c) => (c ? "charging" : "discharging")).get()]} tooltipText={chargeTooltip(charging)} iconName={chargeIcon(charging)} />
);
...
const TheLabelReveal = ({ battery, charging }: { battery: AstalBat.Device; charging: any }) => {
	return (
		<revealer transitionType={REVEAL_SLIDE_RIGHT} transitionDuration={TRANSITION} revealChild={bind(battery, "charging").as((c) => !c)}>
			<label label={bind(battery, "percentage").as((p) => `${p * 100}%`)} tooltipText={chargeTooltip(charging)} onDestroy={(self) => self.destroy()} />
		</revealer>
	);
};
...
export default function BatteryButton() {
	...
	return (
		<button
			cssClasses={[batteryButtonClassName()]}
			hexpand
			visible
		...
		>
			<box halign={CENTER} valign={CENTER} spacing={3}>
				{[TheLabelReveal({ battery: Bat, charging: Charging }), ChargeIndicatorIcon({ battery: Bat, charging: Charging }), BatteryLevelBar(Percentage.get(), PowerProfiles)]}
			</box>
		</button>
	);
}
