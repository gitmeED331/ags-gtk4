...
function GridCalendar() {
	...
	function header() {
		const prevMonth = (
			<button halign={CENTER} valign={CENTER} cssClasses={["arrow-left"]} on_clicked={() => changeMonth(-1)}>
				<image iconName={"arrow-back-circle-symbolic"} />
			</button>
		);

		const nextMonth = (
			<button halign={CENTER} valign={CENTER} cssClasses={["arrow-right"]} on_clicked={() => changeMonth(1)}>
				<image iconName={"arrow-forward-circle-symbolic"} />
			</button>
		);
		...
		const returnToday = (
			...
			>
				<image iconName={"nix-snowflake-symbolic"} />
			</button>
		);

		const prevYear = (
			<button halign={CENTER} valign={CENTER} cssClasses={["arrow-left"]} on_clicked={() => changeYear(-1)}>
				<image iconName={"arrow-back-circle-symbolic"} />
			</button>
		);

		const nextYear = (
			<button halign={CENTER} valign={CENTER} cssClasses={["arrow-right"]} on_clicked={() => changeYear(1)}>
				<image iconName={"arrow-forward-circle-symbolic"} />
			</button>
		);
