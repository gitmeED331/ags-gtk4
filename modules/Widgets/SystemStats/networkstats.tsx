...
const NetworkUsage = ({ iface }: { iface: string }) => {
	return <label label={bind(networkStats).as((stats) => `${formatNetworkThroughput(getTotalNetworkThroughput(stats[iface]))}`)} />;
};
...
export const wired = (
	<box cssClasses={["stats", "wired"]} spacing={5}>
		<image iconName={"gnome-dev-ethernet"} />
		<NetworkIndicatorWired iface={"enp1s0"} />
	</box>
);
export const wifi = (
	<box cssClasses={["stats", "wifi"]} spacing={5}>
		<image iconName={"network-wireless-hotspot-symbolic"} />
