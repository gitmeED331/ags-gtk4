...
function AudioElement({ element, type, ...props }: { element: AstalWp.Endpoint; type: "device" | "stream" } & Widget.SliderProps) {
	const Bindings = Variable.derive(
		...
		(volume, isMuted, aIcon, volumeIcon, description, speaker, microphone) => ({
			...
			buttonCN: {
				device: isMuted || volume === 0 ? "muted" : "",
				stream: isMuted || volume === 0 ? "muted" : "",
			}[type],
			sliderCN: {
				device: isMuted || volume === 0 ? "muted" : "",
				stream: isMuted || volume === 0 ? "muted" : "",
		...
		<box vertical spacing={5} halign={CENTER} valign={CENTER}>
			...
			>
				<box spacing={5} valign={FILL} halign={START}>
					...
					<label xalign={0} max_width_chars={28} label={bind(Bindings).as((d) => d.theDescription)} halign={START} />
				</box>
			</button>
			<slider
				cssClasses={["audio-mixer", "Slider", bind(Bindings).as((c) => c.sliderCN).get()]}
				halign={START}
				valign={FILL}
				vexpand={true}
				drawValue={false}
				...
				onChangeValue={({ value, dragging }: any) => {
					if (dragging) {
...
export default function () {
	...
	return (
		<box vertical cssClasses={["audio-mixer", "container"]} spacing={10} hexpand={false}>
			...
			<box cssClasses={["audio-mixer", "streams"]} vertical>
