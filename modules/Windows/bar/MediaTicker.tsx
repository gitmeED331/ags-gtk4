...
function tickerButton(player: Mpris.Player) {
	const CustomLabel = ({ info }: { info: "tracks" | "artists" }) => {
		...
		return (
			<label
				cssClasses={["ticker", Bindings.as((b) => b.classname).get()]}
	...
	const theStack = (
		<Stack
			...
			setup={(self) => {
				...
				// 		addNoMediaPage();
				// 	} else if (pages.get_n_items() >= 1) {
				// 		const currentChild = self.get_visible_child_name();
				// 		const currentIndex = pages.((child) => child.name === currentChild);
				// 		const nextIndex = (currentIndex + 1) ;
				// 		self.set_visible_child_name([nextIndex].name);
				// 	}
				// }, 7500);
			}}
		/>
